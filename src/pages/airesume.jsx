...
import { Upload, FileText, Wand2, MessagesSquare, CheckCircle2, Github, ExternalLink, Sparkles } from 'lucide-react'
...
export default function AIResume() {
  const shots = [
    { src: '/AIResume/MainScreen.png', alt: 'Main Page', caption: 'Main Screen' },
    { src: '/AIResume/Upload.png', alt: 'Upload Page', caption: 'Upload Resume' },
    { src: '/AIResume/Output.png', alt: 'Output Page', caption: 'Immediate Insights' }
  ]
    ...
    <main className="relative min-h-screen w-full overflow-hidden bg-gray-950 text-white">
      ...
      <header className="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm/6 tracking-wider text-white/70">AI RESUME CRITIQUER</span>
        </div>
        <div className="flex items-center gap-3">
          <Link
            href="https://github.com/karmdesai13/AI-Resume-Critique.git"
            target="_blank"
      ...
      <section className="relative mx-auto w-full max-w-7xl px-6 pt-6 pb-6 md:pt-10">
        <div className="mx-auto grid max-w-4xl place-items-center text-center">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
            {[{ Icon: FileText, label: 'Instant Feedback' }, { Icon: Upload, label: 'Upload Resume' }, { Icon: MessagesSquare, label: 'Prompt Engineering' }, { Icon: Sparkles, label: 'OpenAI Language Models' }].map(({ Icon, label }) => (
            ...
            <motion.div className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setLightboxIndex(null)}>
              ...
              >
                ...
                <div className="mt-3 text-right">
                  <button onClick={() => setLightboxIndex(null)} className="rounded-xl bg-white/10 px-3 py-1.5 text-sm font-medium text-white hover:bg-white/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-fuchsia-400/60">Close</button>
                </div>
              </motion.div>
            </motion.div>
          )}
      ...
      <section className="mx-auto w-full max-w-7xl px-6 pb-14">
        ...
        <div className="mb-7 flex flex-wrap items-center justify-center gap-2">
          {['Streamlit', 'Prompt Engineering', 'OpenAI Language Models', 'Upload & Immediate Insights', 'Web Application'].map((t) => (
      ...
      <footer className="mx-auto w-full max-w-7xl px-6 pb-10 text-center text-sm text-white/60">© 2025 AIResume Critiquer. All rights reserved.</footer>
